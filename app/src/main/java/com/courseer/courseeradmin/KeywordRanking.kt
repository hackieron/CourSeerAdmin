...
class KeywordRanking : Fragment() {

    private var interests: String? = null // Assuming this variable holds the filtered programs
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                for (document in querySnapshot.documents) {
                    val interestsString = document["interests"] as? String

                    interestsString?.split(",")?.forEach { program ->
    ...
    private fun getColorForProgram(program: String, context: Context): Int {
        ...
        // Assuming interests is a comma-separated string of program names
        val distinctPrograms = interests?.split(",")?.map { it.trim() }?.distinct()
